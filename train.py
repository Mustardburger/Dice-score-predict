...
from models.cnn import ResNet, save_network_config
from tools.lossutils import MSE
from dataset import DiceDataset, get_dataloader, get_dataset_dirs, make_subset_dirs
...
if __name__ == "__main__":
    ...
    training_data = DiceDataset(dir_dict["IMG_TRAIN_DIR"], dir_dict["SEG_TRAIN_DIR"], dir_dict["DICE_TRAIN_DIR"], ratio=parser_args.data_train_ratio, subset_dir=dir_dict["SUBSET_TRAIN_DIR"])
    val_data = DiceDataset(dir_dict["IMG_VAL_DIR"], dir_dict["SEG_VAL_DIR"], dir_dict["DICE_VAL_DIR"], ratio=parser_args.data_val_ratio, subset_dir=dir_dict["SUBSET_VAL_DIR"])
    ...
    # Load model
    # Question here is, how to feed in the input?
    # Concat the seg and img, or element-wise adding?
    # Currently, the channel for both the mri and the seg is 3
    # The paper https://www.sciencedirect.com/science/article/pii/S1361841521002589 concatenates the seg and image to create a 6-channel input
    model = ResNet(parser_args.num_in_channels).to(device)
        ...
        for data in train_dataloader:
            ...
            if parser_args.modality_dropout_rate > 0.0:
                ...
                if ran <= parser_args.modality_dropout_rate:     # Do the dropout
                    mask0, mask_minus1 = create_mask(imgs.size(), parser_args.modality_dropped)
